...
class TodoList:
    ...
    def show(self, context=''):
        ...
        for task in data:
            if not task.done and task.is_relevant_to_context(context) and task.has_started():
                try:
    ...
    def save(self, location):
        ...
        with open(location, 'w', encoding='utf8') as data_f:
            ...
            json.dump(data, data_f, sort_keys=True, indent=4, ensure_ascii=False)
...
def import_data(data_location):
    ...
    for dico in data['tasks']:
        ...
        if 'visibility' not in dico:
            if 'context' in dico and dico['context'] in contexts and 'v' in contexts[dico['context']]:
